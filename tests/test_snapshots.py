...
def _snapshot_test(
    ...
    if result is None:
        # Builder returned None → section is empty; write/compare empty marker
        actual = ""
    else:
        actual = _xml_str(result)
...
def test_snapshot_qemu_guest_agent_basic(update_snapshots: bool) -> None:
    _snapshot_test("qemu_guest_agent", build_qemu_guest_agent, QemuGuestAgentConfig, update_snapshots)
